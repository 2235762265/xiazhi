...
| url() | URL函数接受一个XML文件，该文件设置了 一个SVG滤镜，且可以包含一个锚点来指定一个具体的滤镜元素。<br>例如：<br>filter: url(svg-url\#element-id) |
| initial | 设置属性为默认值，可参阅： CSS initial 关键字 |
| inherit | 从父元素继承该属性，可参阅：<br>CSS inherit 关键字 |
