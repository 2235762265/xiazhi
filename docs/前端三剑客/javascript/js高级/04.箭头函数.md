...
1. 箭头函数对事件处理函数 是可以进行简写的----但是一般是不会使用的
...
1. 一般的确认方式：
   1. 在书写箭头函数的上一行打印this的位置打印的结果，就是箭头函数内部 的 this指向

   1. 箭头函数内部**不仅没有this，而且不能改变this**，因为没有，所以无法改变，**所以 call()  apply()  bind() 方法针对 箭头函数是 无效的**

   1. 箭头函数**适合于无复杂逻辑或者无副作用的纯函数场景下**，例如用在 给一些方法作为 参数 时定义
...
### 3.2 函数的形参只有一个且没有赋值的时候可以不写 () ，其余情况必须写
...
### 3.3函数体只有一行代码的时候，可以不写 {} ，并且会自动 return
