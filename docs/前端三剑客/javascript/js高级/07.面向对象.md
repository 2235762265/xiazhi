...
1. 面向过程是一种以事件为中心的编程思想，编程的时候把解决问题的步骤分析出来，然后用函数把这些步骤实现，在一步一步的具体步骤中再按顺序调用函数。 以过程为核心，强调事件的流程、顺序面向对象

1. 面向对象:

1. 面向对象是一种以 对象 为中心的编程思想，把要解决的问题分解成各个对象，建立对象的目的不是为了完成一个步骤，而是为了描叙某个对象在整个解决问题的步骤中的属性和行为。  以对象为核心，强调事件的角色、主体
...
- 和普通函数一样，只不过 **调用的时候必须要和 new 连用**，不然就是一个普通函数调用
- 构造函数**首字母一般大写**，更加语义化
...
**每次new一个新对象，都会创建一个内存空间，所以每个对象的数据互不影响**
...
当我们new了多个不同的对象，每个对象里都会有相应的属性和方法，我们两个对象内的属性和函数就是代码一摸一样，功能一摸一样

但是是两个空间函数，占用两个内存空间

因此一摸一样的函数出现了两次，占用了两个空间地址，因此就会引起不必要的内存消耗



于是就有了一个新的知识，**原型**

原型相当于一个公共仓库，每个实例对象都可以使用里面的数据
...
- 实例化对象的 __proto__ 和所属的构造函数的 prototype 是同一个对象空间
...
- p1自己没有 sayHi 方法，就会去自己的 __proto__ 中查找
...
我们说构造函数的 prototype 是一个对象 又说了每一个对象都天生自带一个 __proto__ 属性那么 构造函数的 prototype 里面的 __proto__ 属性又指向哪里呢？
...
1. 全局对象有哪些?navigator history location window(面试题)



那么__proto__和prototype到底是什么，两个概念理解它们

1. 函数都有一个属性叫做**prototype** 里面有一个**constructor**属性指向构造函数， 记录了实例的创建者是谁

1. 对象都有一个属性叫做__proto__ 指向构造函数的prototype
...
该方法用于 检测 ob1 是否 存在于 ob2 的 原型链上
...
1.constructor 内添加动态成员

- **constructor构造器 === 和构造函数是一样的**

- 给 实例对象 添加的普通属性（动态成员）书写在 **constructor ( ) { }** 构造器 中,( ) 可以写参数

- 在 constructor构造器 的 this 指向 实例对象
...
        this.name = '张三';
        ...
        //添加 动态方法， 箭头函数和普通函数都行
        this.hello = ()=>{
            console.log(`箭头函数===我是${this.name}，今年${this.age}岁，性别${this.sex}`);
        }
        this.sayHi = function(){
             console.log(`function函数===我是${this.name}，今年${this.age}岁，性别${this.sex}`);
        }
    }
}
//实例对象
let p1=new Person('男',22)
p1.hello()
p1.sayHi()
```
...
    }
    ...
    hei(){         //给原型添加方法
         console.log(this);    //this指向实例对象的原型对象
         console.log('此方法添加在原型中')
    }
}

// 给 原型 添加的 方法
Person.prototype.lala = function(){
    console.log(this);     //this指向实例对象的原型对象，hei()方法效果相同
}
 // 给 原型 添加的 属性
