...
1. 理解：Vue3.0中一个新的配置项，值为一个**函数**。里边书写**js语句**
...
#### 6.2. setup的参数，setup(props,context)
...
defineEmits是一个函数，里面接收一个**数组**，数组中存放**自定义事件名**

```js
const emit = defineEmits(['eventName', 'eventName2']); 
emit('eventName', value1,...,valuen)	// eventName为自定义事件名
```
